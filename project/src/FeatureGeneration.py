...
class DataReaderUSFlu():
    ...
    def GenerateNodeFeatures(self):
        numEventsAll = generate_data(os.path.join(self.path, 'series_for_rnn_3049.csv'))
    ...
    def GenerateNodeClassification(self):
        return partition_to_three_parts(os.path.join(self.path, 'series_for_rnn_3049.csv'))
    ...
    def GenerateGraph(self, threshold=0.01, symmetrize=True, exclude_self=True):
        return load_graph_and_normalize(os.path.join(self.path, 'department_matrix.csv'),
